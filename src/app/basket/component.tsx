  ...
  return (
    <div className={styles.layout}>
      <div className={styles.layoutImage}>
        <Image src={img} layout='responsive' width={200} height={200} loading='eager' alt={title} />
      </div>
      ...
      <div className={styles.layoutQue}>
        <InputNumber value={value} onChange={(e) => (e ? onChange(e) : null)} style={{ pointerEvents: 'visible' }} />
      </div>
      ...
      <div className={styles.close}>
        <Button shape='circle' onClick={() => destroy(id)}>
          <CloseCircleFilled />
        </Button>
      </div>
    </div>
...
const BasketRows = () => {
  ...
  return (
    <>
      {products?.map((item) => (
        <Row key={item.id} item={item} destroy={destroy} />
      ))}
    </>
  );
};
